...
public class Main {
    public static void main(String[] args) {
        ...
        //Завдання 2
        int array0[] = new int[20];//розмір на 20 числ
        for(int i=0; i<20; i++){//i index
            array0[i] = (int)(Math.random() * 10);//рандом числа від 0 до 9
        }
        for(int i=0; i<20; i++) {
            System.out.print(array0[i] + " ");//просто виводимо масим
        }

        for(int i=1; i<20; i++) {
            if (i % 2 != 0) {//перевір на непар
                array0[i] = 0;//непарн присвой знач 0
            }
        }
        System.out.println();
        for(int i=0; i<20; i++) {
        ...
        System.out.println();
        System.out.println(mid1 = mid1 / 5);//шук сер знач
        System.out.println(mid2 = mid2 / 5);
        System.out.println(mid3 = mid3 / 5);
        if (mid1 < mid2 & mid3<mid2)//шук найб сер знач
        ...
        System.out.println();
        for (int i = 0; i < 10; i++) {
            array30[i] = array10[i] + array20[i];//ств масив з сум перших елм друг....
        ...
        //Завдання 5
        int b = 0;
        int array100[] = new int[16];
        for (int i = 0; i < 16; i++) {
            array100[i] = (int)(Math.random() * 10);
            System.out.print(array100[i] + " ");
            if (array100[i] % 2 == 0)//elem/2 if ==0 +1
                b++;//kil vsix par elem
        }
        System.out.println();
        ...
        for(int i=0;i<array12.length;i++){//i рядок j стовпець
            for(int j=0;j<array12[i].length;j++){
                array12[i][j]=(int)(Math.random()*15);//в кожен масив дода ранд числа з 0 до 14
